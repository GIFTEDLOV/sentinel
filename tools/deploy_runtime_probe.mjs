...
const SOURCE_PATH = `${ROOT}/contracts/sentinel_runtime_probe.py`;
const OUT = `${ROOT}/deploy/evidence/final-v4/runtime-diagnosis`;
const JOURNAL = `${OUT}/RUNTIME_PROBE_DEPLOYMENT.json`;
...
journal.entries.push({ label: "runtime probe deployment", tx: deployTx, nonce: before.latestNonce, fees: safe(deployFees) });
await writeFile(JOURNAL, JSON.stringify(safe(journal), null, 2) + "\n", "utf8");
console.log(JSON.stringify({ broadcast: "runtime probe deployment", tx: deployTx, nonce: before.latestNonce }));
const deployReceipt = await waitSameHash(client, deployTx, "runtime probe deployment");
const deployTerminal = requireSuccess(deployReceipt, "runtime probe deployment");
...
await writeFile(JOURNAL, JSON.stringify(safe(journal), null, 2) + "\n", "utf8");
console.log(JSON.stringify({ probeAddress, deployTx, schemaMethods: Object.keys(schema.methods ?? {}).sort() }, null, 2));

const registerArgs = [PROTOCOL_ID, TARGET, FAILURE, DOMAINS, PREFIX, RPC, 2n, 3600n, 900n];
await writeOnce(client, account, journal, "probe register policy", probeAddress, "register_protected_protocol", registerArgs);
await writeOnce(client, account, journal, "probe lock policy", probeAddress, "lock_emergency_policy", [PROTOCOL_ID]);
await writeOnce(client, account, journal, "probe open incident", probeAddress, "open_incident", [INCIDENT, PROTOCOL_ID]);
await writeOnce(client, account, journal, "probe bind chain evidence", probeAddress, "bind_evidence", [INCIDENT, "chain-50de89d7215d", "EMERGENCY", "CHAIN_TRANSACTION", RPC, FAILURE, OUTFLOW_DIGEST, OUTFLOW_TX, 0n, OUTFLOW_INPUT]);
await writeOnce(client, account, journal, "probe bind advisory evidence", probeAddress, "bind_evidence", [INCIDENT, ADVISORY_ID, "EMERGENCY", "SECURITY_ADVISORY", ADVISORY_URL, FAILURE, ADVISORY_DIGEST, OUTFLOW_TX, 1n, ""]);
const finalPreflight = await preflight(client, account, "probe after setup");
journal.after = finalPreflight;
await writeFile(JOURNAL, JSON.stringify(safe(journal), null, 2) + "\n", "utf8");
console.log(JSON.stringify({ complete: true, probeAddress, journal }, null, 2));
