...
function Fact({ label, value, mono = false, copy = false, copyValue }: { label: string; value: ReactNode; mono?: boolean; copy?: boolean; copyValue?: string }): ReactElement {
  ...
  return <div className="fact"><dt>{label}</dt><dd className={mono ? "mono" : ""}>{value}{copy && (copyValue ?? raw) ? <CopyButton value={copyValue ?? raw ?? ""} /> : null}</dd></div>;
}
...
type ReadFailureDetail = { id: number; message: string | UserFacingError; retry?: () => void; hasLastVerified: boolean };
let readFailureId = 0;
function announceReadFailure(detail: Omit<ReadFailureDetail, "id">): void {
  if (typeof window !== "undefined") window.dispatchEvent(new CustomEvent<ReadFailureDetail>("sentinel-read-failure", { detail: { ...detail, id: ++readFailureId } }));
}
...
function Shell({ children }: { children: ReactElement }): ReactElement {
  ...
  const [readFailure, setReadFailure] = useState<ReadFailureDetail | null>(null);
  const pathname = window.location.pathname.replace(/\/+$/, "") || "/";
  const closeMobile = useCallback(() => { setMobileOpen(false); }, []);
  const toggleMobile = useCallback(() => { setMobileOpen((value) => !value); }, []);
  useEffect(() => {
    const onReadFailure = (event: Event) => setReadFailure((event as CustomEvent<ReadFailureDetail>).detail);
    window.addEventListener("sentinel-read-failure", onReadFailure);
    return () => window.removeEventListener("sentinel-read-failure", onReadFailure);
  }, []);
  return <div className="app-shell console-shell">
    {readFailure ? <ReadFailureToast key={readFailure.id} message={readFailure.message} onRetry={readFailure.retry ? () => { setReadFailure(null); readFailure.retry?.(); } : undefined} hasLastVerified={readFailure.hasLastVerified} /> : null}
...
function LoadingPage(): ReactElement { return <div className="page-width loading-state"><span className="spinner" />Reading finalized Studio Next state…</div>; }
...
function ReadFailureToast({ message, onRetry, hasLastVerified = false }: { message: string | UserFacingError; onRetry?: () => void; hasLastVerified?: boolean }): ReactElement | null {
  const [dismissed, setDismissed] = useState(false);
  const normalized = typeof message === "string" ? normalizeUserError(message, "read") : message;
  const error = presentUserError(normalized);
  useEffect(() => { setDismissed(false); }, [message]);
  if (dismissed) return null;
  return <aside className="read-toast" role="status" aria-live="polite"><span className="read-toast-icon" aria-hidden="true">!</span><div className="read-toast-body"><strong>Live state delayed</strong><p>{hasLastVerified ? "The last verified view remains available while Studio Next refreshes." : "Studio Next is taking longer than usual to respond."}</p><div className="read-toast-actions">{onRetry ? <button className="button button-quiet button-small" type="button" onClick={onRetry}>Retry</button> : null}{error.technical ? <details><summary>Details</summary><code>{error.technical}</code></details> : null}</div></div><button className="read-toast-dismiss" type="button" onClick={() => setDismissed(true)} aria-label="Dismiss live data notification">×</button></aside>;
}

function ReadUnavailable({ label, onRetry }: { label: string; onRetry?: () => void }): ReactElement {
  return <section className="read-unavailable" role="status"><span className="read-unavailable-icon" aria-hidden="true">—</span><div><strong>{label}</strong><small>Waiting for a verified Studio Next response.</small></div>{onRetry ? <button className="button button-quiet button-small" type="button" onClick={onRetry}>Retry</button> : null}</section>;
}
...
function useDashboard() {
  ...
  useEffect(() => { let active = true; void readDashboard().then((next) => { if (active) { setData(next); setError(null); } }).catch((reason: unknown) => { if (active) { const message = reason instanceof Error ? reason.message : "Chain read failed."; setError(message); announceReadFailure({ message, retry, hasLastVerified: Boolean(data) }); } }); return () => { active = false; }; }, [attempt]);
...
function LandingPage(): ReactElement {
  ...
  return <div className="landing"><div className="container"><nav className="landing-nav"><a className="brand" href="/" aria-label="Sentinel home"><img className="brand-mark-image" src="/sentinel-sites-mark.png" alt="" /><img className="brand-wordmark" src="/sentinel-sites-wordmark.png" alt="Sentinel. Always Ahead" /></a><div className="landing-links"><a href="#response">Response loop</a><a href="#controls">Controls</a><a href="#integration">Integration</a><a href="/app">Command center <span aria-hidden="true">→</span></a></div><a className="button ghost" href="/app">ENTER APP <span aria-hidden="true">→</span></a></nav><main>
    ...
    <section className="landing-section principle-section" id="purpose"><div className="section-kicker pink-label">THE TRUST BOUNDARY</div><h2>Authentication first.<br />Judgment second.<br /><span>Consequence last.</span></h2><p className="section-lead">Sentinel authenticates objective multi-source evidence before asking GenLayer validators one bounded semantic question. The verdict is one gate in a larger proof: pause, remediation, recovery, and target state are each verified independently.</p></section>
...
function ControlCenter(): ReactElement {
  ...
  if (!dashboard.data) return <><PageIntro kicker="COMMAND CENTER / LIVE" title="Protocol posture" body="The command center remains available while live protocol data catches up." action={<div className="intro-actions"><a className="button button-quiet" href={`/app/incidents/${FINAL_DEPLOYMENT.incidentId}`}>Open live proof</a><a className="button" href="/app/incidents/new">Report incident</a></div>} /><main className="page-width"><ReadUnavailable label="Command center waiting for live state" onRetry={dashboard.retry} /></main></>;
  const { protocol, incidents, target } = dashboard.data;
  if (!protocol) return <><PageIntro kicker="COMMAND CENTER / LIVE" title="Protocol posture" body="The command center remains available while the protected protocol catches up." action={<div className="intro-actions"><a className="button button-quiet" href={`/app/incidents/${FINAL_DEPLOYMENT.incidentId}`}>Open live proof</a><a className="button" href="/app/incidents/new">Report incident</a></div>} /><main className="page-width"><ReadUnavailable label="Protocol state unavailable" onRetry={dashboard.retry} /></main></>;
  const incident = incidents.find((item) => item.incidentId === FINAL_DEPLOYMENT.incidentId) ?? incidents[0];
  return <><PageIntro kicker="COMMAND CENTER / LIVE" title="Protocol posture" body="A chain-derived command view for the protected surface, current incident, and recovery proof." action={<div className="intro-actions"><a className="button button-quiet" href={`/app/incidents/${incident?.incidentId ?? FINAL_DEPLOYMENT.incidentId}`}>Open live proof</a><a className="button" href="/app/incidents/new">Report incident</a></div>} /><main className="page-width"><section className="posture-hero"><div><p className="eyebrow">PROTOCOL STATUS</p><div className="posture-title">{incident?.state ?? "NORMAL"}</div><p className="posture-copy">{incident?.state === "RECOVERED" ? "Recovered and operational. Remediation remains permanently active." : "Current posture is derived from the incident and target readbacks."}</p></div><div className="posture-meta"><Badge value={target?.paused ? "PAUSED" : "RECOVERED"}>{target?.paused ? "Target paused" : "Target operational"}</Badge><span className="mono">{short(FINAL_DEPLOYMENT.protectedDemoAddress)}</span><small>ProtectedDemo readback</small></div></section><section className="dashboard-grid"><Metric label="Target state" value={target?.paused ? "PAUSED" : "OPERATIONAL"} detail={target?.remediated ? "remediation retained" : "direct readback"} emphasis={target?.paused ? "danger" : "good"} /><Metric label="Evidence quorum" value="2 / 2 / 2" detail="bound · fresh · distinct" emphasis="good" /><Metric label="Treasury" value={`${target?.treasuryBalance ?? 0} units`} detail={`${target?.totalOutflow ?? 0} total outflow`} /><Metric label="Policy" value={protocol.policyLocked ? "LOCKED" : "OPEN"} detail={`${protocol.minimumSources} sources · ${protocol.recoveryCooldownSeconds}s cooldown`} emphasis={protocol.policyLocked ? "good" : "warn"} /></section><section className="panel"><div className="panel-heading"><div><p className="eyebrow">LIFECYCLE</p><h2>Full incident governance</h2></div><span className="panel-caption">Every gate is separately verified</span></div><Lifecycle state={incident?.state ?? "NORMAL"} /></section><div className="dashboard-split"><section className="panel"><div className="panel-heading"><div><p className="eyebrow">PROTECTED SURFACE</p><h2>{protocol.protocolId}</h2></div><Link href={`/app/protocols/${protocol.protocolId}`}>Profile →</Link></div><dl className="fact-list"><Fact label="Failure class" value={protocol.criticalFailureClass} /><Fact label="Target" value={short(protocol.targetAddress)} mono copy /><Fact label="Controller" value={target?.controllerConfigured ? "Configured" : "Not configured"} /><Fact label="Processed" value={target?.totalProcessed ?? "—"} /><Fact label="Remediation" value={target?.remediated ? "Retained" : "Not active"} /></dl></section><section className="panel"><div className="panel-heading"><div><p className="eyebrow">LATEST INCIDENT</p><h2>{incident?.incidentId ?? "No incident"}</h2></div>{incident ? <Badge value={incident.state}>{mapIncidentStateToLabel(incident.state)}</Badge> : null}</div>{incident ? <IncidentRow incident={incident} /> : <p className="panel-copy">No configured incident record. Sentinel does not fabricate a feed.</p>}<div className="proof-note"><span>✓</span><p><strong>Live proof available.</strong> Inspect both evidence phases, consensus decisions, target state, and the permanent unsafe-outflow test.</p></div></section></div><section className="panel activity-preview"><div className="panel-heading"><div><p className="eyebrow">VERIFIED ACTIVITY</p><h2>Response record</h2></div><Link href="/app/activity">Full ledger →</Link></div><div className="activity-strip"><ActivityChip label="Incident" value="ACTIVE_INCIDENT" /><ActivityChip label="Containment" value="PAUSED → REMEDIATED" /><ActivityChip label="Recovery" value="SAFE_TO_RECOVER" /><ActivityChip label="Final" value="RECOVERED" /></div></section></main></>;
}
...
function ProtocolsPage(): ReactElement {
  ...
  if (!dashboard.data) return <><PageIntro kicker="PROTOCOL REGISTRY" title="Protected protocols" body="The registry remains available while live protocol data catches up." /><main className="page-width"><ReadUnavailable label="Protocol registry waiting for live state" onRetry={dashboard.retry} /></main></>;
  return <><PageIntro kicker="PROTOCOL REGISTRY" title="Protected protocols" body="An institutional registry of surfaces that have explicitly authorized Sentinel response infrastructure." action={<span className="source-chip">SOURCE · CONFIGURED ID + CHAIN READ</span>} /><main className="page-width single-column"><div className="registry-note"><strong>1 protected surface</strong><span>The current Sentinel contract has no enumeration method; this registry is deliberately limited to the finalized production protocol id.</span></div>{dashboard.data.protocol ? <ProtocolCard protocol={dashboard.data.protocol} target={dashboard.data.target} /> : <ReadUnavailable label="Protocol state unavailable" onRetry={dashboard.retry} />}</main></>;
}
...
function ProtocolDetailPage({ protocolId }: { protocolId: string }): ReactElement {
  ...
  useEffect(() => { let active = true; void Promise.all([readProtocol(), readProtectedDemoState()]).then(([nextProtocol, nextTarget]) => { if (active && nextProtocol?.protocolId === protocolId) { setProtocol(nextProtocol); setTarget(nextTarget); } else if (active) { const message = "Unknown protocol id."; announceReadFailure({ message, retry, hasLastVerified: Boolean(protocol || target) }); } }).catch((reason: unknown) => { if (active) { const message = reason instanceof Error ? reason.message : "Protocol read failed."; announceReadFailure({ message, retry, hasLastVerified: Boolean(protocol || target) }); } }); return () => { active = false; }; }, [protocolId, attempt]);
  if (!protocol) return <><PageIntro kicker="PROTOCOL SECURITY PROFILE" title={protocolId} body="Reading policy and target state." /><main className="page-width"><ReadUnavailable label="Protocol profile unavailable" onRetry={retry} /></main></>;
...
function IncidentCommandCenter({ incidentId }: { incidentId: string }): ReactElement {
  ...
  const [attempt, setAttempt] = useState(0);
  const retry = () => { setAttempt((value) => value + 1); };
  useEffect(() => { let active = true; void readIncidentDossier(incidentId).then((next) => { if (active) setDossier(next); }).catch((reason: unknown) => { if (active) { const message = reason instanceof Error ? reason.message : "Incident read failed."; announceReadFailure({ message, retry, hasLastVerified: Boolean(dossier) }); } }); return () => { active = false; }; }, [incidentId, attempt]);
  if (!dossier) return <><PageIntro kicker="INCIDENT COMMAND CENTER" title={incidentId} body="Reading incident state and evidence." /><main className="page-width"><ReadUnavailable label="Incident proof unavailable" onRetry={retry} /></main></>;
...
function IncidentsPage(): ReactElement {
  ...
  if (!dashboard.data) return <><PageIntro kicker="INCIDENT REGISTRY" title="Incident history" body="The registry remains available while live incident data catches up." /><main className="page-width"><ReadUnavailable label="Incident registry waiting for live state" onRetry={dashboard.retry} /></main></>;
...
function NewIncidentPage(): ReactElement {
  ...
  const retryContext = () => { setContextAttempt((value) => value + 1); };
  useEffect(() => { let active = true; void Promise.all([readProtocol(), readProtectedDemoState()]).then(([nextProtocol, nextTarget]) => { if (!active) return; setProtocol(nextProtocol); setTarget(nextTarget); }).catch((reason: unknown) => { if (active) { const nextError = normalizeUserError(reason, "read"); announceReadFailure({ message: nextError, retry: retryContext, hasLastVerified: Boolean(protocol || target) }); } }); return () => { active = false; }; }, [contextAttempt]);
...
function ActivityPage(): ReactElement {
  const [rows, setRows] = useState<ProofActivity[] | null>(null); const [error, setError] = useState<string | null>(null); const [attempt, setAttempt] = useState(0);
  const retry = () => { setError(null); setAttempt((value) => value + 1); };
  useEffect(() => { let active = true; void readProofActivity().then((next) => { if (active) { setRows(next); setError(null); } }).catch((reason: unknown) => { if (active) { const message = reason instanceof Error ? reason.message : "Activity read failed."; setError(message); announceReadFailure({ message, retry, hasLastVerified: Boolean(rows) }); } }); return () => { active = false; }; }, [attempt]);
  return <><PageIntro kicker="ACTIVITY / PROOF LEDGER" title="Verified system activity" body="A transaction is only complete when finality, consensus, execution, and expected state are all understood." action={<Link href={`/app/incidents/${FINAL_DEPLOYMENT.incidentId}`}>Open incident →</Link>} /><main className="page-width"><section className="activity-legend"><span><i className="legend-dot good" />Finalized + execution verified</span><span><i className="legend-dot warn" />Pending / reconcile same hash</span><span><i className="legend-dot danger" />Execution failed</span></section>{!rows ? error ? <ReadUnavailable label="Activity ledger unavailable" onRetry={retry} /> : <LoadingPage /> : <section className="panel ledger-panel"><div className="ledger-head"><span>OPERATION</span><span>TRANSACTION</span><span>CONSENSUS</span><span>EXECUTION</span><span>STATE</span></div>{rows.map((row) => <div className="ledger-row" key={row.hash}><div><strong>{row.label}</strong><small>{row.stage}</small></div><ProofLink hash={row.hash} /><span className="ledger-value">{row.consensus}</span><span className="ledger-value">{row.execution}</span><Badge value={row.status}>{row.status}</Badge></div>)}</section>}<section className="panel activity-limit"><p className="eyebrow">RECONCILIATION MODEL</p><p>Hashes are persisted before waiting. A timeout is never permission to submit again. The ledger reads known proof transactions from Studio Next; it does not invent historical activity.</p></section></main></>;
}
