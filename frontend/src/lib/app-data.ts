...
import { csvToIds, decodeDecisionInput, decodeEvidence, decodeIncident, decodeProtectedDemoState, decodeProtocol } from "./decoders";
...
async function readDecisionInput(assessmentId: string, transport: GenLayerTransport): Promise<DecisionInput | null> {
  if (!assessmentId) return null;
  try {
    const raw = await transport.readContract<unknown>({
      address: sentinelClientConfig.contractAddress as string,
      method: "get_decision_input",
      args: [assessmentId],
    });
    return decodeDecisionInput(raw);
  } catch {
    // V5 deployments predate decision-input records. Preserve their proof as
    // legacy rather than inventing a commitment for it.
    return null;
  }
}
...
async function readIncidentDossierSnapshot(incidentId: string, options: ReadOptions = { forceFresh: true }): Promise<VerifiedRead<IncidentDossier>> {
  return readSharedVerified(dossierKey(incidentId), readIdentity({ incidentId }), async () => {
    ...
    const [evidence, decisions] = await Promise.all([
      Promise.all(evidenceIds.map((evidenceId) => readEvidenceSnapshot(evidenceId, transport, { forceFresh: true }))),
      Promise.all([
        readDecisionInput(incidentRecord.data.incidentAssessmentId ?? "", transport),
        readDecisionInput(incidentRecord.data.recoveryAssessmentId ?? "", transport),
      ]),
    ]);
    return {
      ...
      authentication: {
        incident: {
          state: incidentRecord.data.incidentAuthenticationState ?? "LEGACY",
          blockCode: incidentRecord.data.incidentAuthenticationBlockCode,
          blockedAt: incidentRecord.data.incidentAuthenticationBlockedAt,
          evidenceCommitment: incidentRecord.data.incidentEvidenceCommitment,
        },
        recovery: {
          state: incidentRecord.data.recoveryAuthenticationState ?? "LEGACY",
          blockCode: incidentRecord.data.recoveryAuthenticationBlockCode,
          blockedAt: incidentRecord.data.recoveryAuthenticationBlockedAt,
          evidenceCommitment: incidentRecord.data.recoveryEvidenceCommitment,
        },
      },
      incidentDecision: decisions[0],
      recoveryDecision: decisions[1],
    };
  }, options);
}
...
const PROOF_LEDGER: Array<[string, string, string | null]> = [
...
async function readProofActivitySnapshot(options: ReadOptions = { forceFresh: true }): Promise<VerifiedRead<ProofActivity[]>> {
  return readSharedVerified(PROOF_ACTIVITY_KEY, readIdentity(), async () => {
    const transport = createStudioNextClient();
    return Promise.all(PROOF_LEDGER.filter((entry): entry is [string, string, string] => Boolean(entry[2])).map(async ([label, stage, hash]) => {
      try {
...
export function invalidateVerifiedReadCaches(incidentId: string = FINAL_DEPLOYMENT.incidentId): void {
