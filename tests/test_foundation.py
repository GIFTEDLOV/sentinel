...
def test_incident_evidence_that_expires_after_binding_fails_closed(
    ...
    assert sentinel.get_incident("incident-1")["incident_authentication_block_code"] == "STALE_EVIDENCE"
...
def test_assessment_fails_when_fresh_evidence_falls_below_minimum(
    ...
    direct_vm.warp("2024-01-02T00:00:01Z")
    sentinel.assess_incident("incident-1")
    assert sentinel.get_incident("incident-1")["incident_authentication_block_code"] == "INSUFFICIENT_EVIDENCE"
...
def test_two_paths_on_one_host_count_as_one_independent_source(
    ...
    sentinel.assess_incident("incident-1")
    assert sentinel.get_incident("incident-1")["incident_authentication_block_code"] == "DUPLICATE_SOURCE"
...
def test_observable_redirect_response_fails_closed(
    ...
    _bind(sentinel, evidence_id="redirected", url="https://explorer.example/tx/redirect", body=body)
    assert sentinel.get_evidence("redirected")["authentication_state"] == "BLOCKED"
...
def test_fetched_content_must_match_bound_digest_and_size_limit(
    ...
    assert sentinel.get_evidence("digest-mismatch")["authentication_block_code"] == "DIGEST_MISMATCH"
    ...
    _bind(sentinel, incident_id="incident-2", evidence_id="oversized", url="https://explorer.example/tx/large", body=oversized_body)
    assert sentinel.get_evidence("oversized")["authentication_state"] == "BLOCKED"
...
def test_wrong_target_evidence_fails_closed(direct_vm, direct_deploy, direct_owner, direct_alice):
    ...
    assert sentinel.get_evidence("wrong-target")["authentication_block_code"] == "TARGET_BINDING_FAILED"
...
def test_unavailable_evidence_fails_closed(direct_vm, direct_deploy, direct_owner, direct_alice):
    ...
    assert sentinel.get_evidence("unavailable")["authentication_state"] == "BLOCKED"
    assert sentinel.get_evidence("unavailable")["authentication_block_code"] == "SOURCE_UNAVAILABLE"
...
def test_recovery_requires_fresh_independent_judgment_and_never_auto_unpauses(
    ...
    assert sentinel.get_incident("incident-1")["recovery_authentication_state"] == "BLOCKED"
...
def test_duplicate_incident_and_insufficient_evidence_transitions_fail(
    ...
    with direct_vm.expect_revert("Incident already exists"):
        _open(sentinel)
    sentinel.assess_incident("incident-1")
    assert sentinel.get_incident("incident-1")["incident_authentication_state"] == "BLOCKED"
    with direct_vm.expect_revert("active incident verdict"):
        sentinel.execute_pause("incident-1")
    with direct_vm.expect_revert("not awaiting target confirmation"):
        sentinel.confirm_pause("incident-1")
...
def test_recovery_evidence_expires_before_assessment(
    ...
    assert sentinel.get_incident("incident-1")["recovery_authentication_block_code"] == "STALE_EVIDENCE"
