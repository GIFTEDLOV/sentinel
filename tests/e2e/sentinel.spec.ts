...
const APP_ROUTES = [
  ...
  "/app/incidents",
  "/app/incidents/new",
  "/app/incidents/incident-hardened-v1-r2-20260917",
  "/app/activity",
  "/transparency",
  "/developer",
] as const;
...
test.describe("Sentinel Studio Next application shell", () => {
  ...
  test("Studio Next network and final deployment addresses are visible", async ({ page }) => {
    ...
    await expect(page.locator(".app-network-chip")).toContainText("Studio Next");
      await expect(page.getByText("0xeA6d5350929d0F3cc80f5f3f9533515C690a3952").first()).toBeVisible();
      await expect(page.getByText("0xdFb4f6Fc4570B04A198b484aa6175025882c7c21").first()).toBeVisible();
  });
  ...
  test("proof page explains the authentication-first stop boundary", async ({ page }) => {
    await page.goto("/transparency");
    await expect(page.getByText("AUTHENTICATION-FIRST BOUNDARY", { exact: true })).toBeVisible();
    await expect(page.getByText("Stop before semantic judgment.", { exact: true })).toBeVisible();
    await expect(page.getByText("Question: INCIDENT_ASSESSMENT_V1", { exact: true })).toBeVisible();
    await expect(page.getByText(/Decision input: 454abba089fb/)).toBeVisible();
  });

  test("new-incident recovery resumes a browser-persisted transaction hash", async ({ page }) => {
    const operationId = `studio-next:61997:0xea6d5350929d0f3cc80f5f3f9533515c690a3952|open_incident|${RECOVERY_INCIDENT_ID}|sentinel-demo`;
  ...
  test("cold cache renders the full app composition before reads resolve", async ({ page }) => {
    await page.addInitScript(() => {
      for (const key of Object.keys(window.localStorage)) {
        if (key.startsWith("sentinel:verified:")) window.localStorage.removeItem(key);
      }
    });
    ...
    await page.goto("/app");
    await expect(page.getByText("PROTOCOL STATUS", { exact: true })).toBeVisible();
    await expect(page.getByText("LATEST INCIDENT", { exact: true })).toBeVisible();
    await expect(page.getByText("VERIFIED ACTIVITY", { exact: true })).toBeVisible();
    await expect(page.getByText("PROTECTED SURFACE", { exact: true })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Response record" })).toBeVisible();
    await expect(page.locator(".loading-state")).toHaveCount(0);
    await expect(page.locator(".value-skeleton").first()).toBeVisible();
    expect(await page.locator(".posture-hero, .latest-incident, .activity-preview, .dashboard-split").count()).toBeGreaterThanOrEqual(4);
  });
