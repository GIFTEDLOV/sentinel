...
const SOURCE = `${ROOT}/diagnostics/hosted_advisory_get_probe.py`;
...
function summary(receipt) {
  return {
    ...
    contractAddress: receipt?.data?.contract_address ?? receipt?.txDataDecoded?.contractAddress ?? null,
  };
}
...
function requireSuccess(receipt, operation) {
  ...
  if (status !== "FINALIZED" || !["MAJORITY_AGREE", "ACCEPTED"].includes(consensus) || execution !== "FINISHED_WITH_RETURN") {
    throw new Error(`${operation} did not finish successfully: ${JSON.stringify(result)}`);
  }
  return result;
}
...
const beforeDeploy = await preflight(client, account);
const currentPolicy = await client.getCurrentFeePolicy();
const deployFees = await client.estimateTransactionFees(options);
if (BigInt(deployFees.feeValue) <= 0n) throw new Error("deployment fee quote is zero");
console.log(JSON.stringify({ event: "DEPLOY_PREFLIGHT", preflight: beforeDeploy, profile, currentPolicy: jsonSafe(currentPolicy), deployFees: jsonSafe(deployFees) }));

const source = await readFile(SOURCE, "utf8");
const deployTx = await client.deployContract({ account, code: source, args: [], fees: deployFees });
console.log(JSON.stringify({ event: "DEPLOY_BROADCAST_RETURNED", evmHash: deployTx }));
await persist({ deploymentEvmHash: deployTx, deploymentPreflight: beforeDeploy, deploymentFees: deployFees });
const deployReceipt = await client.waitForTransactionReceipt({ hash: deployTx, retries: 50, interval: 5000, waitUntil: "decided", fullTransaction: true });
const deployResult = requireSuccess(deployReceipt, "diagnostic deployment");
const diagnosticAddress = deployResult.contractAddress;
if (!diagnosticAddress) throw new Error("diagnostic deployment returned no contract address");
console.log(JSON.stringify({ event: "DEPLOY_RECONCILED", evmHash: deployTx, address: diagnosticAddress, receipt: deployResult }));
await persist({ deploymentEvmHash: deployTx, diagnosticAddress, deploymentPreflight: beforeDeploy, deploymentFees: deployFees, deploymentReceipt: deployResult });

const beforeProbe = await preflight(client, account);
...
console.log(JSON.stringify({ event: "PROBE_SIMULATION", preflight: beforeProbe, probeFees: jsonSafe(probeFees), simulation: jsonSafe(simulation) }));

const beforeProbeBroadcast = await preflight(client, account);
const probeTx = await client.writeContract({ account, address: diagnosticAddress, functionName: "probe", args: [], fees: probeFees });
console.log(JSON.stringify({ event: "PROBE_BROADCAST_RETURNED", evmHash: probeTx }));
await persist({ deploymentEvmHash: deployTx, diagnosticAddress, deploymentPreflight: beforeDeploy, deploymentFees: deployFees, deploymentReceipt: deployResult, probePreflight: beforeProbeBroadcast, probeFees, probeEvmHash: probeTx });
const probeReceipt = await client.waitForTransactionReceipt({ hash: probeTx, retries: 100, interval: 5000, waitUntil: "decided", fullTransaction: true });
const probeResult = requireSuccess(probeReceipt, "hosted advisory GET probe");
console.log(JSON.stringify({ event: "PROBE_RECONCILED", evmHash: probeTx, receipt: probeResult }));
await persist({ deploymentEvmHash: deployTx, diagnosticAddress, deploymentPreflight: beforeDeploy, deploymentFees: deployFees, deploymentReceipt: deployResult, probePreflight: beforeProbeBroadcast, probeFees, probeEvmHash: probeTx, probeReceipt: probeResult, result: "PASS" });
