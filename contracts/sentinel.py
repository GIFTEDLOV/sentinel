...
class Sentinel(gl.contract.Contract):
    ...
    def _authentication_matches(self, leader_result: typing.Any, validator_result: typing.Any) -> bool:
        ...
        for field in (
            ...
            if leader_result.get(field) != validator_result.get(field):
                return False
        return leader_result.get("status") == AUTH_VALID
    ...
    def _authenticate_for_binding(
        ...
        if not isinstance(result, dict) or result.get("status") != AUTH_VALID:
            raise gl.vm.UserError("Evidence authentication failed")
        return result
...
"""
    ...
    def _build_assessment_context(
        ...
        if bound_count < minimum_sources:
            raise gl.vm.UserError("Bound evidence is insufficient for assessment")
        if authenticated_count < minimum_sources:
            raise gl.vm.UserError("Authenticated evidence is insufficient for assessment")
        if fresh_count < minimum_sources:
            raise gl.vm.UserError("Fresh evidence is insufficient for assessment")
        if distinct_count < minimum_sources:
            raise gl.vm.UserError("Distinct evidence sources are insufficient for assessment")
    ...
    def bind_evidence(
        ...
        # agreement. The caller cannot provide or extend this timestamp.
